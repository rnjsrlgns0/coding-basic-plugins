...
## Plugin 관리
이 프로젝트는 **개발(development)**, **개발-임시(development-temp)**, **데이터 사이언스(data-science)** 세 가지 플러그인으로 구성되어 있습니다.
...
- **development-temp**: 간소화된 개발 워크플로우 (Serena MCP 기반)
  - 에이전트: backend-api-developer, frontend-ui-developer, ui-ux-designer, web-app-tester, code-debugger, technical-documentation-writer
  - 커맨드: branch, complete, plan, update-memory, workflow
  - MCP 서버: serena (코드베이스 분석, 심볼 탐색, 메모리 관리)
...
- **development-temp**: 간소화된 개발 프로젝트용 (Serena MCP 서버 기반 코드 분석)
